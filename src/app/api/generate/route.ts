...
export async function POST(request: NextRequest) {
  try {
    ...
    const prompt = `
다음 원본 텍스트를 바탕으로 "${selectedType.type}" 형식의 문서를 작성해주세요.

원본 텍스트: "${text}"

문서 제목: ${selectedType.title}

목차 구조:
${outline.map((item: string, index: number) => `${index + 1}. ${item}`).join('\n')}
...
`;
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: '당신은 전문적인 문서 작성 전문가입니다. 주어진 텍스트와 구조를 바탕으로 완성도 높은 문서를 작성합니다.'
        },
        {
          role: 'user',
          content: prompt
